...
import com.tehbeard.pasta.schematic.worker.ISchematicWorker;
        ...
        for(int y = 0;y<file.getHeight();y++){
            for(int x = 0;x<file.getWidth();x++){
                for(int z = 0;z<file.getLength();z++){
                    ...
                    if(Block.byId[b_id] != null || b_id == 0){
                        world.setTypeIdAndData(v.getX(), v.getY(), v.getZ(), b_id, b_meta,2);
                    }
                    else
                    {
                        if(b_id !=0){
        ...
        for(int y = 0;y<file.getHeight();y++){
            for(int x = 0;x<file.getWidth();x++){
                for(int z = 0;z<file.getLength();z++){
                    ...
                    if(b_id != maskId &&
                            (world.getTypeId(v.getX(), v.getY(), v.getZ()) !=  b_id ||
                            world.getData(v.getX(), v.getY(), v.getZ()) != b_meta)
                            ){
                        return false;
                    }
